...
# ---------------------------------------------------------------------------
# 1. Bulk create validation with type confusion
# ---------------------------------------------------------------------------


class TestBulkCreateTypeConfusion:
    def test_bulk_create_title_null_rejects(self, client):
        """Bulk create where one item has title: null — should 400."""
        _post_task(client)
        token = _register(client)
        resp = client.post(
            "/api/tasks/r6-task/items/bulk",
            json={"items": [{"title": "valid"}, {"title": None}]},
            params={"token": token},
        )
        assert resp.status_code == 400

    def test_bulk_create_labels_integer_rejects(self, client):
        """Bulk create where one item has labels: 123 (integer, not array) — should 400."""
        _post_task(client)
        token = _register(client)
        resp = client.post(
            "/api/tasks/r6-task/items/bulk",
            json={"items": [{"title": "valid"}, {"title": "other", "labels": 123}]},
            params={"token": token},
        )
        assert resp.status_code == 400

    def test_bulk_create_status_array_rejects(self, client):
        """Bulk create where one item has status: [] (array) — should 400."""
        _post_task(client)
        token = _register(client)
        resp = client.post(
            "/api/tasks/r6-task/items/bulk",
            json={"items": [{"title": "valid"}, {"title": "other", "status": []}]},
            params={"token": token},
        )
        assert resp.status_code == 400

    def test_bulk_create_atomicity_third_item_fails(self, client):
        """Bulk create: 3rd of 5 items has title: null — first 2 must NOT be committed."""
        _post_task(client)
        token = _register(client)
        items = [
            {"title": "item 1"},
        ...
        list_resp = client.get("/api/tasks/r6-task/items")
        assert list_resp.status_code == 200
        assert list_resp.json()["items"] == [], (
            "Atomicity violated: items were committed despite 3rd item failing"
        )
...
class TestConcurrentBulkOperations:
    def test_two_bulk_creates_unique_sequential_ids(self, client):
        """Two bulk creates of 10 items each — verify all 20 get unique sequential IDs with no gaps."""
        _post_task(client)
        token = _register(client)

        batch_a = {"items": [{"title": f"batch-a-{i}"} for i in range(10)]}
        batch_b = {"items": [{"title": f"batch-b-{i}"} for i in range(10)]}

        resp_a = client.post(
            "/api/tasks/r6-task/items/bulk", json=batch_a, params={"token": token}
        )
        assert resp_a.status_code == 201

        resp_b = client.post(
            "/api/tasks/r6-task/items/bulk", json=batch_b, params={"token": token}
        )
        ...
        assert len(set(all_ids)) == 20, f"Expected 20 unique IDs, got {len(set(all_ids))}: {all_ids}"
        expected = {f"R6-{i}" for i in range(1, 21)}
        assert set(all_ids) == expected, f"Expected IDs 1-20, got {set(all_ids)}"

    def test_bulk_create_then_bulk_update_to_done_then_list(self, client):
        """Bulk create 5, bulk update all to status=done, list with status=done — all 5 returned."""
        _post_task(client)
        token = _register(client)

        batch = {"items": [{"title": f"item-{i}"} for i in range(5)]}
        resp = client.post(
            "/api/tasks/r6-task/items/bulk", json=batch, params={"token": token}
        )
        ...
        update_resp = client.patch(
            "/api/tasks/r6-task/items/bulk",
            json={"items": [{"id": iid, "status": "done"} for iid in item_ids]},
            params={"token": token},
        )
        assert update_resp.status_code == 200

        list_resp = client.get("/api/tasks/r6-task/items", params={"status": "done"})
        assert list_resp.status_code == 200
        returned_ids = {item["id"] for item in list_resp.json()["items"]}
        assert returned_ids == set(item_ids), (
            f"Expected all 5 items to be returned with status=done, got {returned_ids}"
        )
...
class TestIdempotencyAndDoubleOps:
    ...
    def test_bulk_update_id_only_no_updatable_fields(self, client):
        """Bulk update with 0 updatable fields per item (only 'id' key) — document behavior."""
        _post_task(client)
        token = _register(client, "r6-zero-fields")
        r = _create_item(client, token=token)
        assert r.status_code == 201
        ...
        # With only 'id' key and no updatable fields, should either succeed (200)
        # or reject with 400 — must not 500
        assert resp.status_code in (200, 400), (
            f"Bulk update with id-only should be 200 or 400, got {resp.status_code}"
        )
