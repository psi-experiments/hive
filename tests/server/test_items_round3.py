...
class TestBulkEdgeCases:
    def test_bulk_create_empty_array_rejects(self, client):
        """Bulk create with empty items array — should 400."""
        _post_task(client)
        token = _register(client)
        resp = client.post(
            "/api/tasks/r3-task/items/bulk",
            json={"items": []},
            params={"token": token},
        )
        assert resp.status_code == 400

    def test_bulk_create_missing_items_key_rejects(self, client):
        """Bulk create with missing items key — should 400."""
        _post_task(client)
        token = _register(client)
        resp = client.post(
            "/api/tasks/r3-task/items/bulk",
    ...
    def test_bulk_update_nonexistent_item_id(self, client):
        """Bulk update with an item ID that doesn't exist — should 404."""
        _post_task(client)
        token = _register(client)
        _create_item(client, token=token)
        resp = client.patch(
            "/api/tasks/r3-task/items/bulk",
            json={"items": [{"id": "R3-9999", "status": "done"}]},
            params={"token": token},
        )
        assert resp.status_code == 404

    def test_bulk_update_one_fails_validation_rollback(self, client):
        """Bulk update where one item has invalid status — whole batch should fail."""
        _post_task(client)
        token = _register(client)
        _create_item(client, token=token)
        _create_item(client, token=token)
        resp = client.patch(
            ...
            json={
                "items": [
                    {"id": "R3-1", "status": "done"},
                    {"id": "R3-2", "status": "not-a-valid-status"},
                ]
            },
            params={"token": token},
        )
        assert resp.status_code == 400
        # Verify R3-1 was NOT updated (rollback occurred)
        item = client.get("/api/tasks/r3-task/items/R3-1").json()
        assert item["status"] != "done"

    def test_bulk_create_one_invalid_label_nothing_created(self, client):
        """Bulk create where one item has invalid labels — verify nothing was created."""
        _post_task(client)
        token = _register(client)
        resp = client.post(
            "/api/tasks/r3-task/items/bulk",
            json={
                "items": [
                    {"title": "Valid item", "labels": ["good-label"]},
                    {"title": "Invalid item", "labels": ["bad label!"]},
                ]
            },
            params={"token": token},
        )
        ...
        list_resp = client.get("/api/tasks/r3-task/items")
