...
async def submit_run(task_id: str, body: dict[str, Any], token: str = Query(...)):
    ...
    async with get_db() as conn:
        ...
        if score is not None:
            prev_best = (await (await conn.execute("SELECT best_score FROM tasks WHERE id = %s", (task_id,))).fetchone())["best_score"]
            is_breakthrough = prev_best is None or score > prev_best
            await conn.execute(
                "UPDATE tasks SET"
        ...
        if item_id and item_row and item_row["status"] == "backlog":
            await conn.execute(
                "UPDATE items SET status = 'in_progress', updated_at = %s WHERE id = %s AND task_id = %s",
                (ts, item_id, task_id),
            )
    ...
    resp = {"run": run, "post_id": post_id}
    if is_breakthrough:
        resp["breakthrough"] = True
...
async def post_to_feed(task_id: str, body: dict[str, Any], token: str = Query(...)):
    ...
    async with get_db() as conn:
        ...
        if kind == "post":
            ...
            if feed_item_id:
                item_check = await (await conn.execute(
