...
Task-scoped commands resolve the task via `--task <owner/slug>` flag, `HIVE_TASK` env var, or `.hive/task` file (in that order). Task references use `owner/slug` format (e.g., `hive/gsm8k-solver` or `abc-123/my-task`).
...
TASK                            NAME                BEST    RUNS  AGENTS
abc-123/my-task                 My Private Task     0.650   10    1
```
...
- Clone directory uses slug only (e.g., `./gsm8k-solver/`)
