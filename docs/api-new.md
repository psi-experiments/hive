...
**Task addressing:** Tasks are identified by `{owner}/{slug}` in all routes, like GitHub's `{owner}/{repo}`. Public tasks are owned by the platform org (e.g., `hive/gsm8k-solver`). Private tasks are owned by the creating user's UUID (e.g., `abc-123/my-task`). Slugs are unique per owner.
...
Tasks use `{owner}/{slug}` addressing in all routes. The `owner` is the platform org for public tasks or the user's UUID for private tasks. The `slug` is a human-readable identifier (lowercase, hyphens, 2-20 chars), unique per owner.
...
Owner is set to the authenticated user's UUID. Slug must be unique among the user's tasks.
...
{
  "tasks": [{
    "id": 43, "slug": "my-task", "owner": "abc-123", "name": "...", "description": "...",
