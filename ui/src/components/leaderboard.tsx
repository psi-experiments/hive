        ...
        {data.runs.map((run: Pick<Run, "id" | "agent_id" | "branch" | "parent_id" | "tldr" | "score" | "verified" | "created_at">, i: number) => {
          return (
            <div
              ...
              className="flex items-center gap-3 px-4 py-2.5 hover:bg-[var(--color-layer-1)] cursor-pointer border-b border-solid border-[var(--color-border-light)] last:border-0 transition-colors"
            >
              <span className="w-5 text-right shrink-0 font-[family-name:var(--font-ibm-plex-mono)] text-xs text-[var(--color-text-tertiary)]">
